    ...
    <SidePanel title="Search" onClose={onClose}>
      <div className="px-6 py-5">
        ...
        <div className="relative mb-5">
          <span className="material-icons-outlined absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground pointer-events-none" style={{ fontSize: 18 }}>search</span>
          <input
            ref={inputRef}
            type="text"
            value={query}
        ...
        {!query.trim() ? (
          <div className="text-center py-12">
            <span className="material-icons-outlined text-muted-foreground/40 block mb-3" style={{ fontSize: 36 }}>search</span>
            <p className="text-sm text-muted-foreground">Start typing to search across StudioOS</p>
          </div>
        ) : results.length === 0 ? (
          <div className="text-center py-12">
            <span className="material-icons-outlined text-muted-foreground/40 block mb-3" style={{ fontSize: 36 }}>search_off</span>
                  ...
                  {items.map(r => (
                    ...
                    >
                      <div className="w-8 h-8 rounded-lg bg-muted flex items-center justify-center flex-shrink-0">
                        <span className="material-icons-outlined text-muted-foreground" style={{ fontSize: 16 }}>{r.icon}</span>
                      </div>
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-medium truncate">{r.label}</p>
                        <p className="text-xs text-muted-foreground truncate">{r.subtitle}</p>
                      </div>
                      <span className="material-icons-outlined text-muted-foreground/30 group-hover:text-muted-foreground transition-colors" style={{ fontSize: 16 }}>chevron_right</span>
                    </Link>
                  ))}
                </div>
              </div>
