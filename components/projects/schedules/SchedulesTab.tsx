...
import {
  ...
  mockSchedules, SCHEDULE_TEMPLATES, createEmptySection,
                ...
                <>
                  ...
                  <div className="absolute right-0 mt-1 w-56 bg-popover border border-border rounded-xl shadow-lg z-20 py-1 max-h-80 overflow-y-auto">
                    ...
                    {SCHEDULE_TEMPLATES.map(template => (
                      ...
                      >
                        <span className="material-icons-outlined text-muted-foreground" style={{ fontSize: 18 }}>
                          table_chart
                        </span>
                        {template}
                      </button>
            ...
            {schedules.length === 0 ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {schedules.map(schedule => {
                  const productCount = schedule.sections.flatMap(s => s.products).length;
                  const approvedCount = schedule.sections.flatMap(s => s.products).filter(p => p.status === 'Approved').length;
                  const totalCost = schedule.sections.flatMap(s => s.products).reduce((sum, p) =>
                  ...
                  return (
                    <div
                      ...
                      className="group border border-border rounded-xl p-4 hover:border-muted-foreground/30 hover:bg-muted/30 cursor-pointer transition-all"
                    >
                      <div className="flex items-start justify-between mb-3">
                        <div className="flex items-center gap-3">
                          <div className="w-10 h-10 rounded-lg bg-muted flex items-center justify-center flex-shrink-0">
                            <span className="material-icons-outlined text-muted-foreground" style={{ fontSize: 20 }}>
                              table_chart
                            </span>
                          </div>
                          <div>
